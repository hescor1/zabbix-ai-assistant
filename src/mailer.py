...
def html_wrap(title, body_html):
    ...
    return (
        ...
        f"<div class=\"footer\">Generated by Zabbix SRE Assistant<br>{ts}</div>"
        f"</div></body></html>"
    )
...
def render_noc_html():
    ...
    ts = datetime.now().strftime("%Y-%m-%d %H:%M")
    html = f"<h1>NOC Operations Report</h1><p>{ts} - Problems accionables: <b>{len(problems)}</b></p>"
    if not problems:
        html += '<div class="ok">No actionable problems. Operations stable.</div>'
    ...
    for sev in sorted(by_sev.keys(), reverse=True):
        ...
        html += "<table><thead><tr><th>Status</th><th>Age</th><th>Host</th><th>Domain</th><th>Problem</th></tr></thead><tbody>"
        for p in items:
            ack = "ACK" if p["acknowledged"] else '<span class="ack-pending">UNACK</span>'
    ...
    if no_ack > 0:
        html += f'<div class="insight">{no_ack} de {len(problems)} problems unacknowledged.</div>'
    return html_wrap("Reporte NOC", html)
...
def render_executive_html():
    ...
    html = f"<h1>Executive Summary</h1><p>{data['timestamp']}</p>"
    html += "<h2>Key Indicators</h2>"
    html += f'<div class="metric"><div class="metric-label">Active problems</div><div class="metric-value">{data["total_active"]}</div></div>'
    html += f'<div class="metric"><div class="metric-label">Disaster</div><div class="metric-value sev-disaster">{data.get("disaster_count", 0)}</div></div>'
    html += f'<div class="metric"><div class="metric-label">High</div><div class="metric-value sev-high">{data.get("high_count", 0)}</div></div>'
    html += f'<div class="metric"><div class="metric-label">Unattended > 7 days</div><div class="metric-value sev-disaster">{data["critical_aging"]}</div></div>'
    html += f'<div class="metric"><div class="metric-label">MTTR 24h</div><div class="metric-value">{format_duration(data["mttr_24h"])}</div></div>'
    html += f'<div class="metric"><div class="metric-label">Resolved by team</div><div class="metric-value">{data.get("resolved_by_team_24h", 0)}</div></div>'
    html += f'<div class="metric"><div class="metric-label">Auto-recovered</div><div class="metric-value">{data.get("auto_recovered_24h", 0)}</div></div>'
    html += "<h2>Overall Status</h2>"
    if data["critical_aging"] > 0:
        html += f'<div class="insight">WARNING: {data["critical_aging"]} problem(s) unattended for more than 7 days.</div>'
    elif data["high_disaster"] > 0:
        html += f'<div class="insight">{data["high_disaster"]} High/Disaster problem(s) active. NOC is tracking.</div>'
    else:
        html += '<div class="ok">Operations stable. No critical problems active.</div>'
    if data["mttr_24h"] is not None and data["mttr_24h"] > 3600:
        html += f'<div class="insight">MTTR ({format_duration(data["mttr_24h"])}) above SRE target (1h for High/Disaster).</div>'
    from signals import get_noc_report
    from collections import Counter
    problems = get_noc_report()
    if problems:
        by_resp = Counter(p["responsible"] for p in problems)
        html += "<h2>Actionable Problems by Owner</h2>"
        html += "<table><thead><tr><th>Owner</th><th>Problems</th><th>High</th><th>Disaster</th></tr></thead><tbody>"
        for resp, count in sorted(by_resp.items(), key=lambda x: -x[1]):
            high = sum(1 for p in problems if p["responsible"] == resp and p["severity"] == 4)
            disaster = sum(1 for p in problems if p["responsible"] == resp and p["severity"] == 5)
            high_html = f'<span class="sev-high">{high}</span>' if high > 0 else "0"
            dis_html = f'<span class="sev-disaster">{disaster}</span>' if disaster > 0 else "0"
            html += f"<tr><td><b>{resp}</b></td><td>{count}</td><td>{high_html}</td><td>{dis_html}</td></tr>"
        html += "</tbody></table>"
    return html_wrap("Executive Summary", html)
...
def render_sre_html():
    ...
    html = f"<h1>SRE Full Report</h1><p>{report.get('timestamp','')} - Active problems: <b>{report.get('total',0)}</b></p>"
    for signal in SIGNAL_ORDER:
        ...
        html += "<table><thead><tr><th>Severity</th><th>Pattern</th><th>Hosts</th><th>Distribution</th></tr></thead><tbody>"
        for p in s["patterns"]:
            sev_lbl = SEV_LABELS.get(p["severity_max"], "?")
            sev_cls = SEV_CLASS.get(p["severity_max"], "info")
            ack = f' <span class="ack-pending">[{p["not_acked"]} unacknowledged]</span>' if p["not_acked"] > 0 else ""
            html += f'<tr><td class="sev-{sev_cls}">{sev_lbl}</td><td>{p["name"]}{ack}</td><td>{p["host_count"]}</td><td>{p["groups"]}</td></tr>'
        html += "</tbody></table>"
    html += "<h2>MTTR (Mean Time to Repair)</h2>"
    html += f'<div class="metric"><div class="metric-label">Last 24h</div><div class="metric-value">{format_duration(mttr["mttr_24h"])}</div></div>'
    html += f'<div class="metric"><div class="metric-label">Last 7 days</div><div class="metric-value">{format_duration(mttr["mttr_7d"])}</div></div>'
    html += f'<div class="metric"><div class="metric-label">Resolved 24h</div><div class="metric-value">{mttr["resolved_24h"]}</div></div>'
    html += "<h2>Aging - Active problems</h2>"
    html += "<table><thead><tr><th>Age</th><th>Count</th><th>Percentage</th></tr></thead><tbody>"
    ...
    return html_wrap("SRE Full Report", html)
...
def render_admin_html():
    ...
    html = f"<h1>Monitoring Quality</h1><p>{data['timestamp']} - Total hosts: <b>{data['total_hosts']}</b></p>"
    html += f'<h2>Unsupported items - {len(data["unsupported_items"])}</h2>'
    if data["unsupported_items"]:
        ...
        html += "<table><thead><tr><th>Host</th><th>Affected items</th></tr></thead><tbody>"
        for host, items in sorted(by_host.items(), key=lambda x: -len(x[1])):
            html += f"<tr><td>{host}</td><td>{len(items)}</td></tr>"
        html += "</tbody></table>"
    else:
        html += '<div class="ok">All items reporting correctly.</div>'
    html += f'<h2>Noisy triggers (3+ veces en 24h) - {len(data["noisy_triggers"])}</h2>'
    if data["noisy_triggers"]:
        html += "<table><thead><tr><th>Count</th><th>Host</th><th>Trigger</th></tr></thead><tbody>"
        for t in data["noisy_triggers"][:15]:
            html += f'<tr><td>{t["count"]}</td><td>{t["host"]}</td><td>{t["name"]}</td></tr>'
        html += "</tbody></table>"
    else:
        html += '<div class="ok">No noisy triggers detected.</div>'
    return html_wrap("Monitoring Quality", html)
...
def send_report_email(report_type):
    report_map = {
        ...
        "ejecutivo": (render_executive_html, "Executive Summary"),
